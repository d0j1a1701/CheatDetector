...
public class Rotation extends Module {
    ...
    public void onFinallyPacketSend(ConnectionAccessor connection, Packet<?> basePacket, @Nullable PacketSendListener packetSendListener, CallbackInfo ci, boolean bl) {
        ...
        if (basePacket instanceof ServerboundMovePlayerPacket packet) {
            ...
            if (basePacket instanceof Pos) {
                send(connection, new Pos(packet.getX(player.fabricPlayer.getX()), packet.getY(player.fabricPlayer.getY()), packet.getZ(player.fabricPlayer.getZ()), packet.isOnGround()), packetSendListener, bl);
            } else if (basePacket instanceof Rot) {
                send(connection, new Rot(getYaw(packet), getPitch(packet), packet.isOnGround()), packetSendListener, bl);
            } else if (basePacket instanceof PosRot) {
                send(connection, new PosRot(packet.getX(player.fabricPlayer.getX()), packet.getY(player.fabricPlayer.getY()), packet.getZ(player.fabricPlayer.getZ()), getYaw(packet), getPitch(packet), packet.isOnGround()), packetSendListener, bl);
            } else if (basePacket instanceof StatusOnly) {
                send(connection, new StatusOnly(packet.isOnGround()), packetSendListener, bl);
            }
        }
    ...
    private void send(@NotNull ConnectionAccessor connection, Packet<?> packet, @Nullable PacketSendListener packetSendListener, boolean bl) {
        sending = packet;
        connection.doSendPacket(packet, packetSendListener, bl);
    }
